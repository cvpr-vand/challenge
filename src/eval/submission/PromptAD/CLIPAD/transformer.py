...
class V2VTransformer(nn.Module):
    ...
    def __init__(
        ...
        # for i in range(0, 12):
            # print("layer", i)
            # self.transformer.resblocks[i].register_forward_hook(hook_t1)
    ...
    def _global_pool(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # print("global_average_pool: ", self.global_average_pool)
        # print("output[1] before pooled: ", x.shape)
        if self.global_average_pool:
            return x.mean(dim=1), x
        else:
            return x[:, 0], x[:, 1:]
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.attn == None:
            ...
            for i in range(1, 25):
                ...
                # print("self.transformer.resblock.shape_of_", -i, self.transformer.resblocks[-i].shape)
