...
def get_dir_from_args(TASK, root_dir, **kwargs):
    ...
    # csv_path = os.path.join(csv_dir, f"results.csv")
    # check_path = os.path.join(check_dir, f"CLS-{kwargs['class_name']}-check_point.pt")
    csv_path = os.path.join(csv_dir, f"Seed_{seed}-results.csv")
    check_path = os.path.join(check_dir, f"CLS-Seed_{seed}-{kwargs['class_name']}-check_point.pt")
