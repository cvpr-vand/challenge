        ...
        if self.gdino_init:
            ...
            if self.class_name == "splicing_connectors":
                if len(clusted_masks) > 0:
                    ...
                    # 直线问题
                    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(clusted_masks[0])
        ...
        if self.class_name == "juice_bottle":
            return ImageBatch(
                ...
                pred_score=torch.tensor(
                    ...
                    40 * anomaly_map_ret_eva02.max().item() +
                    10 * eva02_anomaly_map_ret_part.max().item() +
                    10 * abs(area_hist_avg.min() - 1) +
                    10 * abs(area_hist_avg.max() - 1) +
                    30 * abs(color_hist_avg.min() - 1) +
            ...
            return ImageBatch(
                ...
                pred_score=torch.tensor(
                    ...
                    30 * eva02_anomaly_map_ret_part.max().item() +
                    35 * eva02_subcategory_mean_dists.max().item() +
            ...
            return ImageBatch(
                ...
                pred_score=torch.tensor(
                    30 * anomaly_map_ret_dinov2.max().item() +
                    45  * anomaly_map_ret_eva02.max().item() +
                    10 * abs(area_hist_avg.min() - 1) +
    ...
    def process_subcategory(self, cat_id, normal_dino_patches, heatmap_cache, stage="test", k=1):
        ...
        def get_features():
            for j in range(k):
                ...
                mask_stack = F.interpolate( thresh, size=self.perform_grid_size, mode="bilinear", align_corners=True ).reshape(self.perform_grid_size**2)
                if mask_stack.sum() < 1:
                    selected = None
                else:
